  ...
  const renderMessage = (message: UIMessage) => {
    return message.parts.map((part, index) => {
      ...
      if (part.type === "tool-reverseQuestion") {
        ...
        if (!questionInput) {
          return <span key={index} className="text-muted-foreground">質問を準備しています...</span>;
        }

        if (part.state === 'output-available') {
          const output = part.output as { answer?: string } | undefined;
          return (
            <div key={index} className="rounded-md border border-emerald-500/20 bg-emerald-500/10 p-3 text-sm">
        ...
        if (part.state === 'output-error') {
          return (
            <div key={index} className="rounded-md border border-destructive/20 bg-destructive/10 p-3 text-sm">
              <p className="font-semibold">{questionInput.question}</p>
              <p className="mt-1 text-destructive">回答処理でエラーが発生しました。もう一度お試しください。</p>
            </div>
          );
        }
        ...
        if (part.state === 'input-available') {
          if (
            ...
            return (
              <div key={index} className="rounded-md border border-yellow-500/20 bg-yellow-500/10 p-3 text-sm">
        ...
        return <span key={index} className="text-muted-foreground">質問内容を受信中...</span>;
      }
      return null
    })
  }
